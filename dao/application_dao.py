...
def get_all() -> List[Application]:
    ...
    with db.SessionLocal() as session:
        return (
            ...
            # Вантажимо і кандидата, і вакансію одразу
            .options(
...
def get_by_vacancy(vacancy_id: int) -> List[Application]:
    ...
    with db.SessionLocal() as session:
        return (
            session.query(Application)
            .options(selectinload(Application.candidate)) # <--- КРИТИЧНО ВАЖЛИВО для /by-vacancy/
...
def create_application(application: Application) -> Application:
    ...
    with db.SessionLocal() as session:
        ...
        # Перечитуємо об'єкт із завантаженням зв'язків
        return (
            session.query(Application)
            .options(
                selectinload(Application.candidate),
