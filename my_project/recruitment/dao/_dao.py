...
def get_by_candidate(candidate_id: int) -> List[Application]:
    ...
    (Тобто на які вакансії подався цей кандидат).
    """
    with db.SessionLocal() as session:
        return (
...
def get_by_vacancy(vacancy_id: int) -> List[Application]:
    ...
    (Тобто які кандидати подалися на цю вакансію).
    """
    with db.SessionLocal() as session:
        return (
            session.query(Application)
...
def create_application(application: Application) -> Application:
    ...
    Оскільки це не клас BaseDAO, пишемо метод create вручну або робимо клас.
    Але згідно твого прикладу machine_product_dao.py, тут функції.
    """
    with db.SessionLocal() as session:
